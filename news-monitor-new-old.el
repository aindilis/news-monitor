...
(defun news-monitor-article-mode-hook ()
 ""
 ;; (gnus-eval-in-buffer-window gnus-article-buffer (news-monitor-article-mode))

 ;; (add-hook 'gnus-select-article-hook 'news-monitor-article-mode-hook)

 ;; define some mode map keybindings to assist us with article tasks,
 ;; but don't do a derived mode because it doesn't seem to work easily
 )
...
\\{w3m-mode-map}"
 (define-key news-monitor-w3m-mode-map "!" 'kmax-not-yet-implemented)
 )
